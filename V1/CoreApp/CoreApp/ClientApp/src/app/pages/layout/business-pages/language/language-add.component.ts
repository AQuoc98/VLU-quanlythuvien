...
export class LanguageAddComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,
    private coreLanguageApiServices: CoreLanguageApiServices,
  ...
  private saveData() {
    ...
    if (this.fControls.id.value == Guid.empty) {
      // Add
      this.coreLanguageApiServices.insert(this.dataModel).subscribe((res) => this.saveDataCompleted(res));
    } else {
      // Update
      this.coreLanguageApiServices.update(this.dataModel).subscribe((res) => this.saveDataCompletedEdit(res))
    }
  }
  ...
  private loadDataEdit(id: string) {
    this.isEdit = true;
    this.coreLanguageApiServices.getById(id).subscribe((res) => {
