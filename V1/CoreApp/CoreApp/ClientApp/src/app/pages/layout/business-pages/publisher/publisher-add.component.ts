...
export class PublisherAddComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,
    private corePublisherApiService: CorePublisherApiService,
  ...
  private saveData() {
    ...
    if (this.fControls.id.value == Guid.empty) {
      // Add
      this.corePublisherApiService.insert(this.dataModel).subscribe((res) => this.saveDataCompleted(res));
    } else {
      // Update
      this.corePublisherApiService.update(this.dataModel).subscribe((res) => this.saveDataCompletedEdit(res))
    }
  }
  ...
  private loadDataEdit(id: string) {
    this.isEdit = true;
    this.corePublisherApiService.getById(id).subscribe((res) => {
