...
export class FormatAddComponent implements OnInit {
  ...
  private saveData() {
    ...
    if (this.fControls.id.value == Guid.empty) {
      // Add
      this.coreFormatApiServices.insert(this.dataModel).subscribe((res) => this.saveDataCompleted(res));
    } else {
      // Update
      this.coreFormatApiServices.update(this.dataModel).subscribe((res) => this.saveDataCompletedEdit(res))
    }
  }
  ...
  private loadDataEdit(id: string) {
    this.isEdit = true;
    this.coreFormatApiServices.getById(id).subscribe((res) => {
