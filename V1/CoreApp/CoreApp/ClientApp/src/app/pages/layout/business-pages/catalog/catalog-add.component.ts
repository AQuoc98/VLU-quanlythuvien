...
export class CatalogAddComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,
    private coreCatalogApiServices: CoreCatalogApiServices,
  ...
  private saveData() {
    ...
    if (this.fControls.id.value == Guid.empty) {
      // Add
      this.coreCatalogApiServices.insert(this.dataModel).subscribe((res) => this.saveDataCompleted(res));
    } else {
      // Update
      this.coreCatalogApiServices.update(this.dataModel).subscribe((res) => this.saveDataCompletedEdit(res))
    }
  }
  ...
  private loadDataEdit(id: string) {
    this.isEdit = true;
    this.coreCatalogApiServices.getById(id).subscribe((res) => {
