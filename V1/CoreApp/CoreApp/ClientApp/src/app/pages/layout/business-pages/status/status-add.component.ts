...
export class StatusAddComponent implements OnInit {
  ...
  private saveData() {
    ...
    if (this.fControls.id.value == Guid.empty) {
      // Add
      this.coreStatusApiServices.insert(this.dataModel).subscribe((res) => this.saveDataCompleted(res));
    } else {
      // Update
      this.coreStatusApiServices.update(this.dataModel).subscribe((res) => this.saveDataCompletedEdit(res))
    }
  }
  ...
  private loadDataEdit(id: string) {
    this.isEdit = true;
    this.coreStatusApiServices.getById(id).subscribe((res) => {
